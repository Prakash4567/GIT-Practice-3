	...
	public String approveDeny(int leaveId, int empMgrId, String status, String mgrComment) throws SQLException {
		...
		if (empMgrId==mgr) {
			if (status.toUpperCase().equals("YES")) {
				...
				pst.setInt(3, leaveId);
				pst.executeUpdate();
				int bal = employee.getEmpLeaveBalance() + leave.getLeaveNoOfDays();
				employee.setEmpLeaveBalance(bal);
				cmd = "update Employee set EMP_LEAVE_BAL=EMP_LEAVE_BAL+? where EMP_ID=?";
				PreparedStatement pst1= con.prepareStatement(cmd);
				pst1.setInt(1, leave.getLeaveNoOfDays());
				pst1.setInt(2, empId);
				pst1.executeUpdate();		
