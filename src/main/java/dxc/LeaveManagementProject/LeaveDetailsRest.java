...
public class LeaveDetailsRest {
	...
	@POST
	@Path("/approveDeny/{leaveId}/{empMgrId}/{status}/{mgrComments}")
	@Produces(MediaType.APPLICATION_JSON)
	public String approveDeny(@PathParam("leaveId") int leaveId, @PathParam("empMgrId") int empMgrId,
			                   @PathParam("status") String status, @PathParam("mgrComment") String mgrComment) throws SQLException {
		String result = new LeaveDAO().approveDeny(leaveId, empMgrId, status, mgrComment);
		return result;
	}
//	@GET
